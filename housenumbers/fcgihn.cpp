...
int initialize(const char* basename) {
  char str[STRSIZ];
  sprintf(str, "fcgi%s.log", basename);
  fpLog = (fopen(str, "a"));
  LOG("------------ %s -------------\n", basename);
  ...
  for (i = 1; i < argc; i++) {
    ...
    if (*pc == '-') {
      ...
      switch (*pc++) {
        ...
        default:
        usage:
          LOG("Usage: fcgihn -bbasename\n");
          GOTO_ERROR;
      }
      continue;
    }
  }
  if (!basename) GOTO_ERROR;
  if (initialize(basename) < 0) GOTO_ERROR;
